<script lang="ts">
	...
	import { HeartRateCard, SocialButtons } from '$lib/components/site';
	...
	// 打字机文本配置
	const heroTexts: string[] = ['PROGRAM CAMILLE', 'ROBOTICS LEARNING', 'TECHNICAL BLOGGER'];
	...
	<div class="w-full max-w-[1920px] mx-auto px-4 md:px-6">
		<div class="grid grid-cols-1 lg:grid-cols-6 gap-2 mt-2 md:mt-2 mb-12">
			...
			<div class="lg:col-span-4 space-y-3 order-2 lg:order-1">
				...
				<h1
					class="text-2xl font-bold md:text-5xl bg-gradient-to-r from-slate-900 via-slate-700 to-stone-500
					   dark:from-slate-100 dark:via-slate-300 dark:to-stone-400 text-transparent bg-clip-text min-h-[3rem] md:min-h-[4rem] flex items-center"
				>
					<TypewriterText
						texts={heroTexts}
						typeSpeed={80}
						deleteSpeed={20}
						delayBetween={2000}
						cursorColor="text-slate-500"
						startDelay={1000}
					/>
				</h1>

				<p>
					A postgraduate student from China. I specialize in Robotics Learning. I want to be a
					full-stack developer. I'm currently seeking software developer roles to further my career.
					In my free time, I enjoy writing technical blogs and sharing my journey with others. If
					you'd like to get in touch, feel free to reach out to me.
				</p>
			...
			<!-- Avatar Section -->
			<div
				class="lg:col-span-2 flex items-center justify-center lg:justify-end order-1 lg:order-2 mb-6 lg:mb-0"
			>
				<Avatar
		...
		<div class="grid grid-cols-1 lg:grid-cols-5 gap-2 mb-8 items-stretch">
			...
			<div class="lg:col-span-1">
				<div class="grid gap-2">
					<div
						class="bg-white/90 dark:bg-slate-950/90 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300"
					>
