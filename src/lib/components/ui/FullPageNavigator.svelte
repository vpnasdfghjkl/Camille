	...
	{#each sections as section, index}
		<div class="group relative">
			...
			>
				...
				<!-- 小箭头 -->
				<div
					class="absolute left-full top-1/2 transform -translate-y-1/2
			...
			<button
				class="w-2.5 h-2.5 rounded-full transition-all duration-300 ease-out
					...
					? 'bg-slate-800 dark:bg-slate-200 shadow-md shadow-slate-900/20 scale-150'
					: 'bg-gray-300/60 dark:bg-gray-600/60 hover:bg-slate-500/70 dark:hover:bg-slate-300/70'}"
				on:click={() => {
					if (!isScrolling) {
