...
每次拿到一台新的云服务器（Ubuntu），用以下脚本快速配置开发环境：
...
# ---------- 1. 安装基础工具 ----------
apt-get update -qq
apt-get install -y -qq zsh direnv tmux rsync
...
# ---------- 4. .zshrc 追加配置 ----------
MARKER="# === camille setup ==="
if ! grep -q "$MARKER" ~/.zshrc 2>/dev/null; then
    # 添加 zsh-autosuggestions 到 plugins
...
cd /root/camille/code/logicmimic
EOF
fi

# ---------- 5. 禁用 conda 自动激活 + 自动切 zsh ----------
if [ -f ~/.bashrc ]; then
    sed -i 's/^conda activate/#&/' ~/.bashrc
    EXEC_ZSH='if [ -x "$(command -v zsh)" ] && [ -z "$ZSH_VERSION" ]; then exec zsh -l; fi'
    grep -q 'exec zsh' ~/.bashrc || sed -i "/\[ -z \"\$PS1\" \] && return/a\\\\n# 自动切换到 zsh\\n$EXEC_ZSH" ~/.bashrc
fi
...
# ---------- 7. 项目目录 + direnv ----------
PROJECT_DIR="/root/camille/code/logicmimic"
mkdir -p "$PROJECT_DIR"
cat > "$PROJECT_DIR/.envrc" << 'EOF'
export VIRTUAL_ENV=/root/camille/venv/il
export PATH="$VIRTUAL_ENV/bin:$PATH"
EOF
direnv allow "$PROJECT_DIR"

echo "✅ Done. 重新登录或执行 exec zsh 生效。"
```

脚本做了这些事：
1. **基础工具**：安装 zsh、direnv、tmux、rsync
2. **Shell**：切换默认 shell 为 zsh，安装 oh-my-zsh + autosuggestions 插件
3. **zshrc**：自动追加 direnv hook、PATH 配置、默认 cd 到项目目录
4. **bashrc**：禁用 conda 自动激活，登录时自动切到 zsh
5. **tmux**：开启鼠标支持
6. **direnv**：自动配置项目虚拟环境（venv）