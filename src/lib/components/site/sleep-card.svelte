<script lang="ts">
	...
	let reading: SleepReading | null = null;
	let isLoading = true;
	let error = '';
	...
	async function loadSleep() {
		try {
			...
			if (!response.ok || !result.success) {
				throw new Error(result.error || 'Failed to load sleep data');
			}
	...
	onMount(() => {
		loadSleep();
		const interval = window.setInterval(loadSleep, 60_000);
...
>
	<div class="flex items-center gap-3 mb-4">
		<div class="p-2 rounded-lg bg-indigo-500/10 text-indigo-500 ring-1 ring-indigo-500/20">
			<Moon size={18} />
		</div>
		<div>
	...
	{:else if error}
		<div class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
			Waiting for sleep sync.
		</div>
	{:else if reading}
		<div class="flex items-end gap-2">
			<span class="text-4xl font-bold tracking-tight text-indigo-500">
				{formatDuration(reading.durationMinutes)}
			</span>
		</div>
		<div
			class="mt-3 flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider text-slate-500"
		>
			<span class="h-2 w-2 rounded-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.55)]" />
			<span>updated {formatUpdatedAt(reading.measuredAt)}</span>
		</div>
	{:else}
		<div class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
			No sleep data yet. Run the iPhone shortcut once to publish your latest sleep duration.
		</div>
	{/if}
</div>
